...
for SUN RGB-D (37-class semantic segmentation).

Expected directory layout::

    {data_root}/
        RGB/          # img-000001.jpg, ...
        Depth/        # img-000001.png, ...
        labels/       # img-000001.png, ...   (注意小写)
        train.txt
        test.txt

Ground-truth labels are 1-indexed in the PNG files; we subtract 1 so that
class indices are 0-based (0 .. 36), and the original 0 (unlabelled)
...
class SUNRGBD(data.Dataset):
    """SUN RGB-D RGBD segmentation dataset.

    Args:
        data_root: Path to dataset root (contains RGB/, Depth/, labels/).
    ...
    def __init__(
        ...
        logger.info(
            f"[SUNRGBD] split={split}, images={len(self._file_names)}, "
    ...
    def __getitem__(self, index: int) -> Dict[str, object]:
        ...
        label_path = os.path.join(self.data_root, "labels", name + ".png")
        ...
        # Depth: single-channel -> 3-channel
    ...
    def _remap_label(label: np.ndarray) -> np.ndarray:
        """Convert 1-indexed SUN labels to 0-indexed; map 0 -> 255 (ignore)."""
